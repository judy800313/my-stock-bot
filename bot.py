...
def main():
    ...
    # 測試通訊用：每組開始跑都會先發一則 LINE
    send_line(f"🤖 機器人啟動：正在掃描第 {group_idx} 組股票...")

    hit_list = []
    # 使用 download 模式減少被擋機率
    try:
        ...
        for s in target:
            try:
                # 處理多檔下載後的資料結構
                df = data['Close'][s].dropna()
                if len(df) < 10: continue
                
                curr = df.iloc[-1]
                high_df = data['High'][s].dropna()
                past_high = high_df.iloc[-8:-1].max()
    ...
    if hit_list:
        send_line(f"🚩【第 {group_idx} 組符合條件】\n" + "\n".join(hit_list))
    else:
        send_line(f"💡 第 {group_idx} 組掃描完畢，無符合條件標的。")
